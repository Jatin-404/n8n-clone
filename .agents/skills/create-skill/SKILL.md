...
|----------|-------------|
| **`.Codex/skills/<name>/` in this repo** | Default for n8n: team-shared, versioned. **Cursor picks up project skills from here** when working in the repo (same idea as Codex). |
| `~/.Codex/skills/<name>/` | Personal skill for Codex across all projects. |
...
Prefer **repo `.Codex/skills/`** for anything that should match how the rest of the team works.
...
5. **Examples** — follow an existing skill in `.Codex/skills/` if one fits.
