...
2. **Check for implementation plan**: Look for a plan file in `.Codex/plans/`
   that matches the current branch's ticket ID (e.g. if branch is
   `scdekov/PAY-1234-some-feature`, check for `.Codex/plans/PAY-1234.md`).
...
If a matching plan file was found in `.Codex/plans/` and the user has approved
