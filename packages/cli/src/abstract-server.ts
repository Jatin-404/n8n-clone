	...
	async init(): Promise<void> {
		...
		this.server.on('error', (error: Error & { code: string }) => {
			if (error.code === 'EADDRINUSE') {
				...
				this.logger.error(
					`WorkflowAI's port ${port} is already in use. Do you have another instance of WorkflowAI running already?`,
				);
				...
				this.logger.error(
					`WorkflowAI does not have permission to use port ${port}. Please run WorkflowAI with a different port.`,
				);
			} else if (error.code === 'EAFNOSUPPORT') {
				// EAFNOSUPPORT is thrown when the address is not available
				this.logger.error(
					`WorkflowAI's address '${address}' is not available. Please run WorkflowAI with a different address, provide correct address in the environment variables N8N_LISTEN_ADDRESS and/or N8N_WORKER_SERVER_ADDRESS.`,
				);
			} else {
				// Other errors are unexpected and should be logged
				this.logger.error('WorkflowAI webserver failed, exiting', {
		...
		this.logger.info(`WorkflowAI ready on ${address}, port ${port}`);
	}
