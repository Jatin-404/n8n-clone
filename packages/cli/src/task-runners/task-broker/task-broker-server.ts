	...
	private async setupHttpServer() {
		...
		this.server.on('error', (error: Error & { code: string }) => {
			if (error.code === 'EADDRINUSE') {
				this.logger.info(
					`WorkflowAI Task Broker's port ${port} is already in use. Do you have another instance of WorkflowAI running already?`,
				);
				process.exit(1);
			}
		});
		...
		this.logger.info(`WorkflowAI Task Broker ready on ${address}, port ${port}`);
	}
