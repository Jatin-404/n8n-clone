...
export class I18nClass {
	...
	private readonly brandTokenRegex = /(?<![@./\w-])n8n(?![\w-])/gi;

	private get i18n() {
		return i18nInstance.global;
	}

	private applyBranding(text: string): string {
		return text.replace(this.brandTokenRegex, 'WorkflowAI');
	}
	...
	baseText(key: BaseTextKey, options?: BaseTextOptions): string {
		...
		result = this.applyBranding(result);
	...
	private dynamicRender({ key, fallback }: { key: string; fallback?: string }) {
		const value = this.i18n.te(key) ? this.i18n.t(key).toString() : (fallback ?? '');
